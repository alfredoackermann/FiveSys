...
namespace Retaguarda.Application.Admin.Test
{
    ...
    public class TipoIdentificacaoTest
    {
        private readonly InicializacaoFixture fixture;
        private readonly ITipoIdentificacaoAppService appService;
        ...
        [InlineData(1, "Tipo de Identificacao 1")]
        [InlineData(2, "Tipo de Identificacao 2")]
        [InlineData(3, "Tipo de Identificacao 3")]
        [InlineData(4, "Tipo de Identificacao 4")]

        public void InserirRegistroValido(int contador, string descricao)
        {
            var resultado = appService.Inserir(new TipoIdentificacaoViewModel()
            {
                Descricao = descricao
            });
        ...
        public void InserirRegistroDuplicado()
        {
            ...
            var resultado = appService.Inserir(new TipoIdentificacaoViewModel()
            {
                Descricao = "Tipo de Identificacao 3"
        ...
        public void Atualizar(string descricao)
        {
            ...
            foreach (var parametro in parametros)
            {
                ...
                var tipoIdentificacao = resultado.Data;

                Assert.NotNull(tipoIdentificacao);

                tipoIdentificacao.Descricao = descricao;

                var resultadoAtualizar = appService.Atualizar(tipoIdentificacao);

                Assert.True(resultadoAtualizar.Successo);
                Assert.Equal(Textos.Geral_Mensagem_Sucesso_Alteracao, resultadoAtualizar.Mensagem);

                resultado = appService.RecuperarPorId(tipoIdentificacao.Id);

                Assert.True(resultado.Successo);

                var tipoIdentificacaoAlterado = resultado.Data;

                Assert.NotNull(tipoIdentificacaoAlterado);

                Assert.Equal(descricao, tipoIdentificacao.Descricao);
