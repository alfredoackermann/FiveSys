...
namespace Retaguarda.Application.Admin.Test
{
    ...
    public class TipoPrecoTest
    {
        private readonly InicializacaoFixture fixture;
        private readonly ITipoPrecoAppService appService;

        public TipoPrecoTest(InicializacaoFixture fixture)
        {
            this.fixture = fixture;
            this.appService = fixture.Container.GetService<ITipoPrecoAppService>();
        ...
        [InlineData(1, "Tipo de Preço 1")]
        [InlineData(2, "Tipo de Preço 2")]
        [InlineData(3, "Tipo de Preço 3")]
        [InlineData(4, "Tipo de Preço 4")]
        public void InserirRegistroValido(int contador, string descricao)
        {
            var resultado = appService.Inserir(new TipoPrecoViewModel()
            {
                Descricao = descricao
            });
        ...
        [InlineData("Tipo Preço teste 1")]
        [InlineData("Tipo Preço teste 2")]
        [InlineData("Tipo Preço teste 3")]
        [InlineData("Tipo Preço teste 4")]
        public void Atualizar(string descricao)
        {
            ...
            foreach (var parametro in parametros)
            {
                ...
                var tipoPreco = resultado.Data;

                Assert.NotNull(tipoPreco);

                tipoPreco.Descricao = descricao;

                var resultadoAtualizar = appService.Atualizar(tipoPreco);

                Assert.True(resultadoAtualizar.Successo);
                Assert.Equal(Textos.Geral_Mensagem_Sucesso_Alteracao, resultadoAtualizar.Mensagem);

                resultado = appService.RecuperarPorId(tipoPreco.Id);

                Assert.True(resultado.Successo);

                var tipoPrecoAlterado = resultado.Data;

                Assert.NotNull(tipoPrecoAlterado);

                Assert.Equal(descricao, tipoPreco.Descricao);
